...
#	ifndef DISABLE_SEMAPHORES
	while( ! sem_continue_wait_n(our_semaphore, led_token1) )
	{
	}
	if( !sem_continue_wait_n(our_semaphore, led_token1) )
...
#	ifndef DISABLE_SEMAPHORES
	while( ! sem_continue_wait_n(our_semaphore, led_token2) )
	{
	}
	if( !sem_continue_wait_n(our_semaphore, led_token2) )
...
TASK(Shift)
{
	...
	led_token3 = sem_start_wait_n(our_semaphore, 1);
	_delay_ms(10);
	led_token4 = sem_start_wait_n(our_semaphore, 1);
	_delay_ms(10);
	sem_abort_wait_n(our_semaphore, led_token3);
	_delay_ms(10);
	sem_abort_wait_n(our_semaphore, led_token4);
